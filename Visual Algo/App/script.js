let bars = []; //Array to contain bars of random lengths
const def = "#fd0081", chng = "#431f91", finished = "#8ef511", selected = "yellow"; //Different colors for different states of a bar
...
async function setup() {
	...
	document.getElementById("d").innerText = d.value + "ms";
		...
		const arr = (b.value).split(" ");
		generateBars(arr.length, arr);
}
...
function generateBars(n, arr) {
	bars = [];
	let container = document.getElementById("container");
	// n = n < 0 ? Math.random() * 20 : n;
	for (let i = 0; i < n; i++) {
		let val = arr[i];
		bars.push('<div class="bar" id="' + i + '" style="height:' + parseInt(arr[i])*1 + '%">'+val+'</div>');
	}
